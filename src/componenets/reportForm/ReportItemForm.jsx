...
const ReportItemForm = React.memo(
  ({ onClose, onSubmit, isUpdate = false, initialData = {} }) => {
      ...
      async (e) => {
        ...
        try {
          ...
          const sanitizedFormData = {
            ...
            category: securityValidation.sanitizeForSubmission(
              formData.category
            ),
            location: securityValidation.sanitizeForSubmission(
              formData.location
            ),
            date: formData.date,
          ...
          // If update mode and initialData.id exists, update the item
          if (isUpdate && initialData && initialData.id) {
            const docRef = doc(db, "items", initialData.id);
            await updateDoc(docRef, itemData);
            setTimeout(() => {
              onSubmit && onSubmit(itemData);
              onClose && onClose();
            }, 1500);
          } else {
